...
async function saveContent() {
  ...
  try {
    ...
    let toSaveMessages = []
    // allow convId from prop OR from message fields (Home may attach convId to message)
    const derivedConvId = props.convId || props.message?.convId || props.message?.conversation_id || null
    let conv = derivedConvId ? historyStore.getConversationById(derivedConvId) : null

    // If conv not found in historyStore, attempt to use a local thread stored on message (fallback)
    // e.g. props.message._localThread = array of messages (home-side)
    const localThread = props.message?._localThread
    ...
    if (props.message) {
      ...
      if (conv && Array.isArray(conv.messages) && conv.messages.length) {
        ...
        if (idx !== -1) {
          ...
          // conv exists but message not found -> fallback to localThread if present
          if (Array.isArray(localThread) && localThread.length) {
            // find index in localThread
            const idxLocal = localThread.findIndex(m => `${m.id}` === `${props.message.id}`)
            if (idxLocal !== -1) {
              if (!isUser) {
                const prev = localThread[idxLocal - 1]
                if (prev && ((prev.role || '').toString().toLowerCase() === 'user')) {
                  toSaveMessages = [prev, localThread[idxLocal]]
                } else {
                  toSaveMessages = [localThread[idxLocal]]
                }
              } else {
                const next = localThread[idxLocal + 1]
                if (next && ((next.role || '').toString().toLowerCase() !== 'user')) {
                  toSaveMessages = [localThread[idxLocal], next]
                } else {
                  toSaveMessages = [localThread[idxLocal]]
                }
              }
        ...
        if (Array.isArray(localThread) && localThread.length) {
          ...
          // 无 conv、无 localThread 时，按原消息为一条
          toSaveMessages = [props.message]
        }
      }
    }
    ...
    let backendOk = false
    try {
      if (userStore?.token && derivedConvId) {
        // 如果带附件或混合类型，需要更复杂逻辑，这里仅支持文本上传（如有文件请改为 uploadMixedInfoService）
        await uploadInfoService({
          conversation_id: derivedConvId,
          text: combined
        })
        backendOk = true
      } else if (userStore?.token && !derivedConvId) {
